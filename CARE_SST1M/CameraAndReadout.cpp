...
/*! \file CameraAndReadout.cpp
    ...
    \author Nepomuk Otte
    \SST-1M adaptation by Alessio Porcelli
*/
...
int main( int argc, char **argv )
{
  ...
  if(readConfig->GetLoopOverEventsBit())
      ...
      if(readConfig->GetVBFwriteBit())
	{
      ...
      //TTree **tClusters = new TTree*[uNumTelescopes];
      for(UInt_t i = 0; i<uNumTelescopes; i++)
	{
	  ...
	  if (readConfig->GetCameraSnapshotUsage(i))
	    {
	      //tClusters[i] = new TTree("TriggerClusters","Trigger clusters for Camera Snapshot logic");
	      tout[i]->Branch("uSnapshots", &(telData[i]->iSnapshots));
	      TString snap_name;
	      for(int s=0; s<telData[i]->iSnapshots; s++)
		{
		  //snap_name.Form("iSnapshot%i",s);
		  //tClusters[i]->Branch(snap_name, &(telData[i]->iSnapshotsDiscriminatedGroups[s]));
		  snap_name.Form("iTriggerClusterSnapshot%i",s);
		  tout[i]->Branch(snap_name, &(telData[i]->iSnapshotsDiscriminatedGroups[s]));
		}
	      //tout[i]->Branch("TriggerClusters", tClusters[i]);
	    }
	}
	  ...
	  if(bSkipEvent)
		...
		{   
		  ...
		  if (SnapshotTrigger)
		    {
		      SnapshotTrigger->RunTrigger();
		      /*if (telData[n]->GetTelescopeTrigger()) {
			cout << " --> Event " << i << " (telescope " << n << "): Triggered!" << endl;
			cout<< "puppa" << endl;}*/
