...
string fVersion = "corsikaIOreader (v 1.42)";
...
// (example corsika: 400 telescocope, grisudet: 49 telescopes)
// grisu cfg file
...
// initialize grisu readers:
            if( fGrisu.size() == 0 )
	    {
	        if( nTel > -2 )
		{
		   fGrisu.push_back( new VGrisu( fVersion ) );
...
// read atmospheric profile from CHERENKOV OPTIONS
 ...
 //(GM) keep Corsika coordinate system            az  = 180. - (180./M_PI)*(evth[11]-evth[92]);   
   ...
   /* Distances of telescopes from shower axis */
	    ...
	    if( bGRISU )
	    {
	        for( unsigned int p = 0; p < fGrisu.size(); p++ ) fGrisu[p]->writeEvent( array );
            }
