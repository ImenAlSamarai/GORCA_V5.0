...
void VIOHistograms::init( string i_outfile, bool iShort )
{
   ...
   if( bSmallFile ) cout << "\t\t smallfile output" << endl; 
   bShort = iShort;
...
// number of Cherenkov photons per telescope
   ...
   if( !bShort )
   {
      if( !bSmallFile ) fTree->Branch( "hGLambda", "TH1D", &hGLambda, 32000,0);  
      fTree->Branch( "hSLambda", "TH1D", &hSLambda, 32000,0);  
      if( !bSmallFile ) fTree->Branch( "hSZeAz", "TH2D", &hSZeAz, 32000,0);

      fTree->Branch( "hBunch", "TH1D", &hBunch, 32000,0);
      fTree->Branch( "hZem", "TH1D", &hZem, 32000,0);  
      fTree->Branch( "hT0", "TH1D", &hT0, 32000,0);

      if( !bSmallFile ) fTree->Branch( "hGXY", "TH2D", &hGXY, 32000,0);
      if( !bSmallFile ) fTree->Branch( "hGZeAz", "TH2D", &hGZeAz, 32000,0);
      if( !bSmallFile ) fTree->Branch( "hGProb", "TH1D", &hGProb, 32000,0);  
      if( !bSmallFile ) fTree->Branch( "hGZem", "TH1D", &hGZem, 32000,0);  

      fTree->Branch( "hSXY", "TH2D", &hSXY, 32000,0);
      if( !bSmallFile ) fTree->Branch( "hSProb", "TH1D", &hSProb, 32000,0);  
...
void VIOHistograms::newEvent( float *evth, telescope_array array, int i_array )
{
   ...
   if( !bShort )
   {
	 ...
	 {
	    if( (it = (TH2D*)hCXYZ->At(i) ) ) it->Reset();
	 }
      }
   }   
...
void VIOHistograms::fillGenerated( bunch ph, double prob )
{
   if( !bShort )
   {
      ...
      az = redang( az );
      if( ph.cy != 0. ) ze = asin( ph.cy / sin( az ) );
      else              ze = asin( ph.cy / cos( az ) );
      ze *= degrad;
      az *= degrad;
      if( az < 0 ) az = 360. - az;

      if( !bCORSIKA_coordinates ) hGXY->Fill( ph.x, ph.y );
      else                        hGXY->Fill( ph.y, -ph.x );
      hGZeAz->Fill( ze, az );
      hGProb->Fill( prob );
...
void VIOHistograms::fillSurvived( bunch ph, double prob )
{
   if( !bShort )
   {
      ...
      if( !bCORSIKA_coordinates ) hSXY->Fill( ph.x, ph.y );
      else                        hSXY->Fill( ph.y, -ph.x );
      hSZeAz->Fill( ze, az );
      hSProb->Fill( prob );
...
// fill xyz histograms
      ...
      {
         if( fXYZlevelsHeight[i] > ph.zem ) continue;
	 ...
	 if( it )
	 {
	    if( !bCORSIKA_coordinates ) it->Fill( xp, yp );
	    else                        it->Fill( yp, -xp );
         }
      }
...
{
   if( iTel < telNumber ) NCp[iTel] = iphotons;
}

void VIOHistograms::terminate()
{
   if( fTree ) fTree->Fill();  // write last event
...
// write all histograms outside of tree if there is only one event in the tree
      if( fTree->GetEntries() == 1 ) hisList->Write();
      fout->Close(); 
   }
   else
   {
