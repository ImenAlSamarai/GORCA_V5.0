...
// (GM)   cout << "Atmospheric extinction model : " << model << endl;
...
// MODTRAN4 US76 extinction values
   ...
   else if( fModel == "noExtinction" )
   {
      ...
      cout << "VAtmosAbsorption: no atmospheric extinction applied" << endl;
   }
   else
   {
...
// check validity of results
        if( !isnormal( atmprob ) )
	{
	   cout << "VAtmosAbsorption::probAtmAbsorbed not normal " << wl << "\t" << atmprob << endl;
	   cout << "\t tlow " << tlow << "\t thigh " << thigh << "\t wemis " << wemis << "\t optdepth " << optdepth << endl;
	   cout << "\t p1 " << p1 << "\t " << p2 << "\t iext_index " << iext_index << "\t iwave2 " << iwave2 << "\t z " << z << "\t fAltitudeStep " << fAltitudeStep << "\t ihgt " << ihgt << endl;
	   cout << "\t ve " << extint.size() << "\t" << extint[iext_index].size() << "\t" << extint[iext_index+1].size() << endl;
	   cout << "\t extint[iext_index][iwave2] " << extint[iext_index][iwave2] << "\t extint[iext_index+1][iwave2] " << extint[iext_index+1][iwave2] << endl;
	   cout << "\t inter " << wave << "\t" << (double)(iwave1*5+minwave) << "\t" << (double)((iwave2)*5+minwave) << "\t" << p1 << "\t" << p2 << endl;
	   return 0.;
        }
	return atmprob;
    }
...
//    cout << "VAtmosAbsorption: reading atmospheric extinction file (from CORSIKA): " << fSourceFile << endl;
...
//   cout << "VAtmosAbsorption: reading atmospheric extinction file (lambda > " << LAMBDA_MIN << " nm): " << fSourceFile << endl;
  ...
  if( finp != NULL )
  {
	  ...
	  {
            cout << "VAtmosAbsorption::read_extint: Error in reading " << fSourceFile;
	    cout << " Lambda expected: " << lambda << "\t Lambda read " << lambda_in << endl;
          }
...
// artemis file without ","
	     ...
	     {
		cout << "Warning: value not normal in " << fSourceFile << " for lambda = " << lambda << " and altitude " << i << " (setting value to 200.)" << endl;
		extint[i][bin] = 200.;
             }             
	  } 
...
// (GM)   cout << "\t reading atmospheric extinction file (from kascade): " << fSourceFile << endl;
...
void VAtmosAbsorption::read_extint_M5(  )
{
  ...
  */
   ...
   if(!is)
   {
	...
	cout << "VAtmosAbsorption::read_extint_M5, reading source file: " << fSourceFile << endl;
   }
	...
	unsigned int i=2;
	 ...
	 while ( is_stream >> temp )
	 {
		...
		if(extint[i].size() < bin+1) {
			...
			cout << "VAtmosAbsorption::read_extint_M5: Overwriting extint[" << i << "][" << bin<< "]" << endl;
